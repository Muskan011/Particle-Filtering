    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        min_a, min_v = None, float('inf')
        max_b = [max( distance, key = lambda x : distance[x]) for distance in livingGhostPositionDistributions]
        for act in legal:
            v = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), b) for b in max_b)
            if v < min_v:
                min_a, min_v = act, v
        return min_a